...
def test_get_users_list(reqres_api: ReqresApi):
    """
    TC-API-01: Verify that GET /users?page=2
    returns 200 and data contains 6 users
    """
...
def test_get_user(reqres_api: ReqresApi):
    """
    TC-API-02: Verify that GET /users/2
    returns 200, data contains
    id = 2 and email = 'janet.weaver@reqres.in.'
    """
    ...
    assert response["data"][
               "email"] == "janet.weaver@reqres.in", f"Expected email to be 'janet.weaver@reqres.in' but got {response["data"]["email"]}"
